"""Tests for the async SQLite persistence layer in `ulysses.tools.db`."""
...
class TestUpsertAndGet:
    async def test_upsert_inserts_new_job(self, db: UlyssesDB) -> None:
        job = await db.upsert_job(_job())
        assert job.id == "job-1"

    async def test_get_job_by_id(self, db: UlyssesDB) -> None:
        await db.upsert_job(_job())
        fetched = await db.get_job("job-1")
        assert fetched is not None
        assert fetched.title == "Python scraper"

    async def test_get_job_returns_none_when_missing(self, db: UlyssesDB) -> None:
        assert await db.get_job("does-not-exist") is None

    async def test_get_job_by_url(self, db: UlyssesDB) -> None:
        await db.upsert_job(_job())
        fetched = await db.get_job_by_url("https://www.upwork.com/jobs/~job-1")
        assert fetched is not None
        assert fetched.id == "job-1"

    async def test_upsert_updates_existing_job(self, db: UlyssesDB) -> None:
        await db.upsert_job(_job())
...
class TestJobExists:
    async def test_true_for_seen_url(self, db: UlyssesDB) -> None:
        await db.upsert_job(_job())
        assert await db.job_exists("https://www.upwork.com/jobs/~job-1") is True

    async def test_false_for_unseen_url(self, db: UlyssesDB) -> None:
        assert await db.job_exists("https://www.upwork.com/jobs/~unseen") is False


class TestListJobs:
    async def test_filters_by_min_score(self, db: UlyssesDB) -> None:
    ...
    async def test_filters_by_category(self, db: UlyssesDB) -> None:
    ...
    async def test_filters_by_status(self, db: UlyssesDB) -> None:
...
class TestProposalDraftsAndPrototypeFiles:
    ...
    async def test_add_and_get_prototype_files(self, db: UlyssesDB) -> None:
        await db.upsert_job(_job())
